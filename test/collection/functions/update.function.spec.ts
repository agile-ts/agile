...
describe('Update Function Tests', () => {
    ...
    MY_COLLECTION.collect({id: 2, name: 'jamie'}, [], {background: true});
    MY_COLLECTION.collect({id: 3, name: 'hans'}, [], {background: true});
    ...
    it('Has correct initial values', () => {
        ...
        expect(JSON.stringify(MY_COLLECTION.data[1].value)).to.eq(JSON.stringify({
            id: 1,
            name: 'jeff'
        }), 'MY_COLLECTION item at id 1 is correct');
        expect(JSON.stringify(MY_COLLECTION.data[2].value)).to.eq(JSON.stringify({
            id: 2,
            name: 'jamie'
        }), 'MY_COLLECTION item at id 2 is correct');
        expect(JSON.stringify(MY_COLLECTION.data[3].value)).to.eq(JSON.stringify({
            id: 3,
            name: 'hans'
        }), 'MY_COLLECTION item at id 3 is correct');

        expect(JSON.stringify(myHookCollection)).to.eq(JSON.stringify([
            {
                id: 1,
                name: 'jeff'
            },
    ...
    it('Can\'t update not existing Item', async () => {
        ...
        expect(rerenderCount).to.eq(2, 'rerenderCount has stayed the same');
    });
