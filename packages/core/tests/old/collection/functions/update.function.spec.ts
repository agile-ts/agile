...
describe("Update Function Tests", () => {
  ...
  it("Can update Item", async () => {
    ...
    expect(Object.keys(MY_COLLECTION.data).length).to.eq(
      3,
      "MY_COLLECTION has still all items"
    );
  ...
  it("Can update Item and primaryKey", async () => {
    ...
    expect(MY_COLLECTION.data[3]).to.eq(
      undefined,
      "MY_COLLECTION item at id 3 doesn't exist anymore"
    );
    expect(Object.keys(MY_COLLECTION.data).length).to.eq(
      3,
      "MY_COLLECTION has still all items"
    ...
    expect(
      ...
      false,
      "MY_COLLECTION item with id 3 doesn't exist in default group anymore"
    );
    expect(
      MY_COLLECTION.groups["default"].value.findIndex(
        (value) => value === 4
      ) !== -1
    ).to.eq(true, "MY_COLLECTION item with id 3 does exist in default group");
