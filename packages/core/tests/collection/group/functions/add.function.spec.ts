...
describe("Add function Tests", () => {
  ...
  it("Can add Group", async () => {
    ...
    expect(
      JSON.stringify(MY_COLLECTION.groups["group1"].notFoundPrimaryKeys)
    ).to.eq(JSON.stringify([]), "group1 has correct notFoundPrimaryKeys");
  ...
  it("Can add item to Group which doesn't exist in collection", async () => {
    ...
    expect(
      JSON.stringify(MY_COLLECTION.groups["group1"].notFoundPrimaryKeys)
    ).to.eq(JSON.stringify([100]), "group1 has correct notFoundPrimaryKeys");

    expect(rerenderCount).to.eq(
      3,
      "rerenderCount stayed the same because output won't change -> rerender not necessary"
    );
  });
  ...
  describe("Test background option", () => {
    ...
    it("Doesn't call callBackFunction by adding Item to group with background = true", async () => {
      ...
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].value)).to.eq(
        JSON.stringify([2, 1, 7, 8, 100, 3, 4]),
        "group1 has correct value"
      );
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].output)).to.eq(
        JSON.stringify([
  ...
  describe("Test method option", () => {
    it("Does add the item at the end of the group with method = 'push'", async () => {
      ...
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].value)).to.eq(
        JSON.stringify([2, 1, 7, 8, 100, 3, 4, 5]),
        "group1 has correct value"
      );
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].output)).to.eq(
        JSON.stringify([
    ...
    it("Does add the item at the start of the group with method = 'unshift'", async () => {
      ...
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].value)).to.eq(
        JSON.stringify([6, 2, 1, 7, 8, 100, 3, 4, 5]),
        "group1 has correct value"
      );
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].output)).to.eq(
        JSON.stringify([
  ...
  describe("Test overwrite option", () => {
    ...
    it("Overwrites existing item and add it at new position with overwrite = true", async () => {
      ...
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].value)).to.eq(
        JSON.stringify([6, 2, 1, 7, 8, 100, 4, 5, 3]),
        "group1 has correct value"
      );
      expect(JSON.stringify(MY_COLLECTION.groups["group1"].output)).to.eq(
        JSON.stringify([
