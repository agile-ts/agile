...
describe("select Function Tests", () => {
  ...
  it("Can select another id which doesn't exist", async () => {
    ...
    expect(MY_COLLECTION.data[100] instanceof Item).to.eq(
      true,
      "MY_COLLECTION at id 100 has been created as placeholder.. to hold connection"
    );
    ...
    expect(MY_COLLECTION.data[100].exists).to.eq(
      false,
