...
describe("Default Collection Tests", () => {
  ...
  describe("Collection", () => {
    ...
    it("Has correct initial values", () => {
      ...
      expect(MY_COLLECTION.key).to.eq(
        undefined,
        "MY_COLLECTION has correct key"
      );
      expect(MY_COLLECTION._key).to.eq(
        undefined,
        "MY_COLLECTION has correct _key"
      );
      ...
      expect(
        Object.keys(MY_COLLECTION.groups).length === 1 &&
          MY_COLLECTION.groups["default"] !== undefined
      ).to.eq(true, "MY_COLLECTION has only default group");
      expect(JSON.stringify(MY_COLLECTION.selectors)).to.eq(
        JSON.stringify({}),
        "MY_COLLECTION has no selectors"
      );
    ...
    describe("Default Groups in Array shape", () => {
      ...
      it("Has correct initial values", () => {
        expect(MY_COLLECTION.groups["group1"] instanceof Group).to.eq(
          true,
          "MY_COLLECTION has group1 in groups"
        );
        expect(MY_COLLECTION.groups["group2"] instanceof Group).to.eq(
          true,
          "MY_COLLECTION has group2 in groups"
        );
        expect(MY_COLLECTION.groups["group1"].key).to.eq(
          "group1",
          "group1 has correct key"
        );
        expect(MY_COLLECTION.groups["group2"].key).to.eq(
          "group2",
          "group2 has correct key"
        );
      });
    });
    ...
    describe("Default Selector in Array shape", () => {
      ...
      it("Has correct initial values", () => {
        expect(MY_COLLECTION.selectors["selector1"] instanceof Selector).to.eq(
          true,
          "MY_COLLECTION has selector1 in selectors"
        );
        expect(MY_COLLECTION.selectors["selector2"] instanceof Selector).to.eq(
          true,
          "MY_COLLECTION has selector2 in selectors"
        );
  ...
  describe("Collection with primaryKey", () => {
    ...
    it("Has correct initial values", () => {
      expect(MY_COLLECTION.config.primaryKey).to.eq(
        "key",
        "MY_COLLECTION has correct primaryKey"
      );
    });
  });
  ...
  describe("Collection with defaultGroupKey", () => {
    ...
    it("Has correct initial values", () => {
      expect(MY_COLLECTION.config.defaultGroupKey).to.eq(
        "normal",
        "MY_COLLECTION has correct defaultGroupKey"
      );
      expect(MY_COLLECTION.groups["normal"] instanceof Group).to.eq(
        true,
        "MY_COLLECTION has default group in groups"
      );
    });
  });
});
