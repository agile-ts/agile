...
describe("createGroup Function Tests", () => {
  ...
  it("Can create Group", () => {
    ...
    expect(MY_COLLECTION.groups["group1"] instanceof Group).to.eq(
      true,
      "MY_COLLECTION group1 has been created"
    );
    expect(JSON.stringify(MY_COLLECTION.groups["group1"].value)).to.eq(
      JSON.stringify([]),
      "group1 has no initialItems"
    );
  ...
  it("Can't overwrite Group which already exists", () => {
    ...
    expect(MY_COLLECTION.groups["group2"] instanceof Group).to.eq(
      true,
      "MY_COLLECTION group2 is still a group"
    );
    expect(JSON.stringify(MY_COLLECTION.groups["group2"].value)).to.eq(
      JSON.stringify(["1", "2"]),
      "group2 items stayed the same"
    );
  });
});
