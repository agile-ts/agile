...
describe("Collect Function Tests", () => {
  ...
  describe("Collection without primaryKey", () => {
    ...
    it("Can collect item", async () => {
      ...
      expect(
        MY_COLLECTION.groups["default"]?.value.findIndex(
          (value) => value === 1
        ) !== -1
      ).to.eq(true, "MY_COLLECTION default group contains collected value");
      expect(MY_COLLECTION.size).to.eq(
        1,
        "MY_COLLECTION size has been increased by 1"
      );
    ...
    it("Can't collect item with wrong primaryKey", async () => {
      ...
      expect(JSON.stringify(MY_COLLECTION.data[2]?.value)).to.eq(
        ...
        "MY_COLLECTION data hasn't collected value"
      );
      expect(
        MY_COLLECTION.groups["default"]?.value.findIndex(
          (value) => value === 2
        ...
        false,
        "MY_COLLECTION default group doesn't contain collected value"
      );
      expect(MY_COLLECTION.size).to.eq(1, "MY_COLLECTION size stayed the same");
    ...
    it("Can collect multiple items at the same time", async () => {
      ...
      expect(
        ...
        true,
        "MY_COLLECTION default group contains collected value with id 2"
      );
      expect(
        MY_COLLECTION.groups["default"]?.value.findIndex(
          (value) => value === 3
        ...
        true,
        "MY_COLLECTION default group contains collected value with id 3"
      );
      expect(MY_COLLECTION.size).to.eq(
        3,
        "MY_COLLECTION size has been increased by 2"
      );
    ...
    it("Can overwrite item if collecting with existing primaryKey", async () => {
      ...
      expect(
        ...
        true,
        "MY_COLLECTION default group still contains collected value"
      );
    ...
    describe("Test background option", () => {
      it("Does call callBackFunction by collecting Item with background = false", async () => {
        // Collect item
        MY_COLLECTION.collect({ id: 4, name: "dustin" }, [], {
          background: false,
        });
        ...
        expect(JSON.stringify(MY_COLLECTION.data[4]?.value)).to.eq(
          JSON.stringify({
            id: 4,
            name: "dustin",
          }),
      ...
      it("Doesn't call callBackFunction by collecting Item with background = true", async () => {
        // Collect item
        MY_COLLECTION.collect({ id: 5, name: "snipey" }, [], {
          background: true,
        });
        ...
        expect(JSON.stringify(MY_COLLECTION.data[5]?.value)).to.eq(
          JSON.stringify({
            id: 5,
            name: "snipey",
          }),
    ...
    describe("Test method option", () => {
      it("Does add the item at the end of the group with method = 'push'", async () => {
        ...
        expect(JSON.stringify(MY_COLLECTION.data[6]?.value)).to.eq(
          JSON.stringify({
            id: 6,
            name: "jana",
          }),
        ...
        expect(
          MY_COLLECTION.groups["default"]?.value[
            MY_COLLECTION.groups["default"]?.value.length - 1
          ]
        ).to.eq(
          6,
          "MY_COLLECTION default group collected value is last item in group"
        );
        expect(MY_COLLECTION.size).to.eq(
          6,
          "MY_COLLECTION size has been increased by 1"
      ...
      it("Does add the item at the start of the group with method = 'unshift'", async () => {
        // Collect item
        MY_COLLECTION.collect({ id: 7, name: "gina" }, [], {
          method: "unshift",
        });
        ...
        expect(JSON.stringify(MY_COLLECTION.data[7]?.value)).to.eq(
          JSON.stringify({
            id: 7,
            name: "gina",
          }),
        ...
        expect(MY_COLLECTION.groups["default"]?.value[0]).to.eq(
          7,
          "MY_COLLECTION default group collected value is first item in group"
        );
        expect(MY_COLLECTION.size).to.eq(
          7,
          "MY_COLLECTION size has been increased by 1"
    ...
    describe("Test patch option", () => {
      it("Does patch value into item without overwriting it with patch = true", async () => {
        ...
        expect(MY_COLLECTION.size).to.eq(
          7,
          "MY_COLLECTION size stayed the same"
        );
    ...
    describe("Test forEachItem option", () => {
      it("Does call forEachItem", async () => {
        ...
        expect(JSON.stringify(MY_COLLECTION.data[8]?.value)).to.eq(
          JSON.stringify({
            id: 8,
            name: "joshi",
          }),
        ...
        expect(JSON.stringify(MY_COLLECTION.data[9]?.value)).to.eq(
          JSON.stringify({
            id: 9,
            name: "paul",
          }),
        ...
        expect(JSON.stringify(myForEachItems[0])).to.eq(
          JSON.stringify({
        ...
        expect(JSON.stringify(myForEachItems[1])).to.eq(
          JSON.stringify({
            item: { id: 9, name: "paul" },
            key: 9,
            index: 1,
