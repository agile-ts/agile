...
describe("Default Computed Tests", () => {
  ...
  describe("Computed", () => {
    ...
    it("Has correct initial values", () => {
      ...
      expect(computedCallCount).to.eq(
        2,
        "computedCallCount has correct initial value"
      );
    ...
    it("Does call computed Function if updating item", async () => {
      ...
      expect(MY_COMPUTED.value).to.eq(
        "hello_hehe_jeff_hans",
        "MY_COMPUTED has correct value"
      );
      expect(computedCallCount).to.eq(
        3,
        "computedCallCount has been increased by 1"
      );
  ...
  describe("Computed with initial subs", () => {
    ...
    it("Has correct initial values", () => {
      ...
      expect(JSON.stringify(MY_COMPUTED.hardCodedDeps)).to.eq(
        JSON.stringify([MY_STATE_3.observer]),
        "MY_COMPUTED has correct hardCodedDeps"
      );
      // @ts-ignore
      expect(JSON.stringify(MY_COMPUTED.deps)).to.eq(
    ...
    it("Does call computed Function if updating item", async () => {
      ...
      expect(rerenderCount).to.eq(
        0,
        "rerenderCount stayed the same because computed hasn't changed"
      );
    });
  });
});
